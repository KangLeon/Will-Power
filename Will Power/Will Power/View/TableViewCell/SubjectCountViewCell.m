...
-(DayView*)dayView_repeat{
    if (!_dayView_repeat) {
        _dayView_repeat=[[DayView alloc] initWithFrame:CGRectMake(self.repeatDay_label.frame.size.width+5, (50-18)/2+9, 40, 18)];
...
-(UIView *)goalView{
    if (!_goalView) {
        _goalView=[[UIView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH-160, 15, 150, 70)];
...
-(UILabel *)goalDiscription{
    if (!_goalDiscription) {
        _goalDiscription=[[UILabel alloc] initWithFrame:CGRectMake(0, 0, 150, 15)];
        //需要判断一下显示
        //1.距离习惯养成还有？天
        //2.坚持目标
